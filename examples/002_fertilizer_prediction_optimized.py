...
def main():
    try:
        ...
        params = {
            ...
            'gpu_use_dp': True,  # Better precision
            'tree_learner': 'feature',  # Better for wide datasets
            'num_leaves': 255,  # Increased for complex features
        ...
        for fold, (train_idx, valid_idx) in enumerate(folds.split(X, y)):
            ...
            model = lgb.train(
                params,
                train_set,
                num_boost_round=3000,  # Increased for complex features
        ...
        feature_imp.to_csv('feature_importance_optimized.csv', index=False)
