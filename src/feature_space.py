...
class FeatureSpace:
    ...
    def generate_generic_features(self, df: pd.DataFrame, check_signal: bool = True, target_column: str = None, id_column: str = None, auto_register: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            if self.generic_operations_config.get('statistical_aggregations', True):
                ...
                stat_features = stat_op.generate_features(df, groupby_cols=groupby_cols, agg_cols=aggregate_cols, auto_register=auto_register)
            ...
            if self.generic_operations_config.get('polynomial_features', True):
                ...
                poly_features = poly_op.generate_features(df, numeric_cols=numeric_cols, degree=degree, auto_register=auto_register)
            ...
            if self.generic_operations_config.get('binning_features', True):
                ...
                bin_features = bin_op.generate_features(df, numeric_cols=numeric_cols, n_bins=n_bins, auto_register=auto_register)
            ...
            if self.generic_operations_config.get('ranking_features', True):
                ...
                rank_features = rank_op.generate_features(df, numeric_cols=numeric_cols, auto_register=auto_register)
            ...
            if self.generic_operations_config.get('categorical_features', True):
                ...
                if categorical_cols:
                    cat_features = cat_op.generate_features(df, categorical_cols=categorical_cols, forbidden_columns=real_forbidden, auto_register=auto_register)
                    ...
                    cat_features = cat_op.generate_features(df, forbidden_columns=real_forbidden, auto_register=auto_register)
                    result_features.update(cat_features)
            ...
            if self.generic_operations_config.get('temporal_features', True):
                ...
                if datetime_cols:
                    temp_features = temp_op.generate_features(df, datetime_cols=datetime_cols, auto_register=auto_register)
            ...
            if self.generic_operations_config.get('text_features', True):
                ...
                if text_cols:
                    text_features = text_op.generate_features(df, text_cols=text_cols, auto_register=auto_register)
    ...
    def generate_custom_features(self, df: pd.DataFrame, dataset_name: str, check_signal: bool = True, auto_register: bool = True) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if hasattr(custom_module, 'CustomFeatureOperations'):
                ...
                custom_features = custom_instance.generate_all_features(df, auto_register=auto_register)
